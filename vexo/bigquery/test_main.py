...
from vexo.bigquery import main
...
def test_iso_canonical_smiles():
    req = _make_request([["C"], ["CC1(C)[C@@H]2C[C@@H](O)[C@@](C)(O)[C@H]1C2"], ["Cc1cc(O)nc(C)n1"], ["CCO/C=N/c1ccccc1"]])
    resp = main.rdkit_iso_canonical_smiles(req)[0]
...
def test_canonical_smiles():
    ...
    req = _make_request([["C"], ["CC1(C)[C@H]2C[C@@H]1[C@](C)(O)[C@H](O)C2"], ["Cc1cc(O)nc(C)n1"], ["CCO/C=N/c1ccccc1"]])
    resp = main.rdkit_canonical_smiles(req)[0]
    resp = json.loads(resp)
...
def test_inchi_inchikey():
    ...
    resp = main.rdkit_inchi_inchikey(req)[0]
    resp = json.loads(resp)
...
def test_smiles_inchikey():
    ...
    resp = main.rdkit_smiles_inchikey(req)[0]
    resp = json.loads(resp)
...
def test_inchi_canonical_smiles():
    ...
    resp = main.rdkit_inchi_canonical_smiles(req)[0]
    resp = json.loads(resp)
...
def test_morgan_fingerprint():
    test_smiles = [["C"], ["CC1(C)[C@H]2C[C@@H]1[C@](C)(O)[C@H](O)C2"], ["Cc1cc(O)nc(C)n1"], ["CCO/C=N/c1ccccc1"]]
    ...
    expected_fp = [AllChem.GetMorganFingerprintAsBitVect(mol[0], useChirality=True, radius=2, nBits=2048) for mol in test_mols]
    expected = [DataStructs.BitVectToBinaryText(fp) for fp in expected_fp]

    resp = main.rdkit_morgan_fingerprint(req)[0]
    resp = json.loads(resp)
...
def test_substructure_match():
    ...
    resp = main.substructure_match(req)[0]
    resp = json.loads(resp)
